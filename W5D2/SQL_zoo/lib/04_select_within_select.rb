...
def richer_than_england
  ...
  SELECT
    ...
    FROM
      countries
    WHERE
      continent = 'Europe' AND (gdp/population) > (SELECT (gdp/population) AS per_capita FROM countries WHERE name = 'United Kingdom') 
  SQL
end
...
def population_constraint
  ...
  FROM
    countries
  WHERE 
   population BETWEEN (SELECT population FROM countries WHERE name = 'Canada') AND 
...
def sparse_continents
  ...
  WHERE continent IN (
    SELECT 
      s1.continent 
    FROM (SELECT continent, count(*) AS c1 FROM countries WHERE population < 25000000 GROUP BY continent) AS s1
    JOIN (SELECT continent, count(*) AS c2 FROM countries GROUP BY continent) AS s2
